document.addEventListener('DOMContentLoaded', function() {
  ...
  function agregarProductoAlDOM(producto) {
    ...
    nuevoProducto.dataset.id = producto.id; // Asegúrate de que el id esté definido
    ...
    botonEliminar.addEventListener('click', function(event) {
      ...
      mostrarConfirmacion(productoAEliminar, () => {
        fetch(`/api/products/${producto.id}`, { // Asegúrate de usar el id correcto aquí
          method: 'DELETE'
        })
        .then(response => {
          if (!response.ok) {
        ...
        .then(() => {
          ...
          productoContainer.removeChild(productoAEliminar);
        })
        .catch(error => console.error('Error:', error));
      });
    });
